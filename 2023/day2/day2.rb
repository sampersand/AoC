...
DATA.each_line do |l|
  l[/(\d+):/]
  $'.split(';').all? do |l|
    q= l.split(',').map{a,b=_1.split;[b,a.to_i]}.to_h#.values_at('green', 'red', 'blue')
    # exit
     q['green'].to_i <= 13 and q['red'].to_i <= 12 and q['blue'].to_i <= 14
    # p f
    # p1 += id if f.sort.map(&:last).zip([14, 13, 12]).all?{ _1<=_2}
    # p1 += 1 if f.values_at('red','green','blue')
    # f.each{f[_1]=[f[_1],_2].max}
  end and p1 += $&.to_i 
end
p p1
=begin
...
# next unless qq == 5
  ...
  id = l[/\d+/].to_i
  f = Hash.new 0
  l[/:\K.*/].split(';').map { _1.strip.split(',').map(&:strip).map(&:split).map(&:reverse).to_h.transform_values(&:to_i)
    .each do |k, v|
      f[k] ||= v
      # p [f, k, v]
      f[k] = [v, f[k]].max
