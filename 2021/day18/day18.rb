...
def reduce(inp)
  ...
  while i < inp.length
    ...
    if depth > 4
      inp.delete_at i # delete the current index

      inp[i-1][0] += val unless i.zero?
      inp[i+1][0] += inp[i][0] if inp[i+1]

      inp[i] =#[0] = 0
      # if inp[i+1] && inp[i+1][1] == depth
      # else
      #   inp[i][1] = depth - 1
      # end
       # = 
       [0, inp[i][1] - 1] # reduce one in depth, 'cause we're splitting

      i = 0
    elsif val >= 10
      half, round = val.divmod 2
      inp[i] = [half, depth + 1]
      inp.insert i+1, [half + round, depth + 1]
      i = 0
    else
      i += 1
    end
...
disp reduce p create p combine [
[1,1],
[2,2],
[3,3],
[4,4],
