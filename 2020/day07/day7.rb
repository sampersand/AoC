BAGS = DATA
	.each_line
	.map { _1.scan(/(^|\d+ )((?!no other)\w+ \w+) bags?/) }
	.map { [_1.shift.last, _1.map(&:reverse).to_h] }
	.to_h

def can_have(color)
	BAGS
		.select { _2.include? color } 
		.map(&:first)
		.map { can_have(_1) << _1 }
		.flatten
		.uniq
...
def bag_count(color)
	1 + BAGS[color].map { bag_count(_1) * _2.to_i }.sum
end

puts "Part 1: #{can_have('shiny gold').count}"
puts "Part 2: #{bag_count('shiny gold') - 1}"
