...
x= Matrix.identity(5)
def step(board)
	Matrix.build board.row_size, board.column_size do |row, col|
		tot = 
			board.minor(
				[row-1, 0].max..[row+1,board.row_size].min,
				[col-1, 0].max..[col+1,board.column_size].min
			).count('#') - [board[row, col]].count('#')

		case
		when board[row, col] == 'L' && tot.zero? then '#'
		when board[row, col] == '#' && tot >= 4 then 'L'
		else board[row, col]
		end
	end
end

board = Matrix[*DATA.map(&:chomp).map(&:chars).reject(&:empty?)]
last, board = board, step(board) while last != board
puts board.count '#'
