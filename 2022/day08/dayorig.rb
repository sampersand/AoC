#! The Jabberwocky was a poem written
#! by Lewis Caroll for his book "Through
#! the Looking-Glass"
...
#! Even though it's not actually valid
#! ruby syntax, it's so cool it can be
#! embedded directly into ruby files
puts "amazing, right?"
p $*
__END__
require 'set'
...
raw2 = <<EOS.lines(chomp: true).map { _1.chars }
30373
25512
65332
33549
...
def scenic(coord)
  score = 1
  coord.real.pred.downto(0).filter_map { $map[coord.imag.i + _1] }
      .count_up { _1.to_i <=> $map[coord].to_i  } * 
    coord.real.succ.upto(100).filter_map { $map[coord.imag.i + _1] }
      .count_up { _1.to_i <=> $map[coord].to_i  } *
    coord.imag.succ.downto(0).filter_map { $map[coord.real + _1.i] }
      .count_up { _1.to_i <=> $map[coord].to_i  } *
    coord.imag.pred.upto(100).filter_map { $map[coord.real + _1.i] }
      .count_up { _1.to_i <=> $map[coord].to_i  }
  # p $map[3+5i]
  # x = coord.real
