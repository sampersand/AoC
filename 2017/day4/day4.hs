...
isuniq :: Eq a => [a] -> Bool
isuniq xs = (length xs) == (length $ uniq xs)

-- pangram :: String -> String -> Bool
-- pangram (x:xs)

pangram :: Eq a => [a] -> [a] -> Bool
pangram l r = any (\x -> any (==x) (permutations r)) (permutations l)

ispangram :: Eq a => [[a]] -> Bool
ispangram xs = any (\x -> any (pangram x) xs) xs

main = do
   contents <- lines <$> readFile "day4.txt"
   let contents' = words <$> contents
   putStrLn $ show $ length $ filter isuniq contents'
   putStrLn $ show $ length $ filter (not . ispangram) contents'
